...
import { IconMedal, IconTarget, IconCheck, IconTrending, IconTrophy } from '../../components/Icons'
...
export default function StudentPortfolio() {
  ...
  // NOTE: was `results.length ? Math.min(...results.filter(r=>r.rank).map(...)) : null` —
  // if results exist but NONE have a rank yet, Math.min(...[]) returns Infinity, not null,
  // and the badge below would have rendered "Best Rank: #Infinity". Guard on the ranked
  // subset's length instead.
  const rankedResults = results.filter(r => r.rank)
  const bestRank = rankedResults.length ? Math.min(...rankedResults.map(r => r.rank)) : null
  ...
  const getLevelColor = (pct) => pct >= 85 ? 'text-emerald-600 bg-emerald-50' : pct >= 70 ? 'text-primary-600 bg-primary-50' : pct >= 55 ? 'text-amber-600 bg-amber-50' : 'text-gray-600 bg-gray-50'

  return (
    <Layout title="My Portfolio">
      {/* Profile hero */}
      <div className="bg-gradient-to-br from-slate-900 to-indigo-950 rounded-3xl p-6 mb-6 text-white">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
          <div className="w-20 h-20 bg-gradient-to-br from-primary-400 to-primary-600 rounded-2xl flex items-center justify-center text-3xl font-black shadow-xl flex-shrink-0">
            {(p.name || user?.email || 'U')[0].toUpperCase()}
          </div>
          <div className="text-center sm:text-left flex-1">
            <h2 className="text-2xl font-black mb-0.5">{p.name || 'Student'}</h2>
            <p className="text-primary-300 text-sm mb-1">{user?.email}</p>
            <div className="flex flex-wrap gap-2 justify-center sm:justify-start text-xs mt-2">
              ...
              {bestRank && (
                <span className="flex items-center gap-1.5 bg-amber-500/20 text-amber-300 px-3 py-1 rounded-full border border-amber-500/30">
                  <IconMedal className="w-3.5 h-3.5" /> Best Rank: #{bestRank}
                </span>
              )}
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard icon={<IconCheck className="w-5 h-5" />}   label="Quizzes Passed" value={passed}                   color="green" />
        <StatCard icon={<IconTrending className="w-5 h-5" />} label="Pass Rate"     value={`${passRate}%`}           color="purple" />
        <StatCard icon={<IconTrophy className="w-5 h-5" />}  label="Best Score"     value={best ? `${best}%` : '—'}  color="orange" />
      </div>
            ...
            <div>
              <div className="flex items-end gap-2 h-32 mb-2">
                {trend.map((r, i) => {
                  ...
                  const color = r.percentage >= 75 ? 'bg-emerald-500' : r.percentage >= 50 ? 'bg-amber-500' : 'bg-rose-400'
                  return (
              ...
              <div className="flex items-center gap-6 mb-4">
                ...
                <div className="space-y-3 flex-1">
                  ...
                  <div>
                    ...
                    <div className="w-full h-2 bg-gray-100 rounded-full"><div className="h-full bg-rose-400 rounded-full" style={{ width: `${total ? ((failed / total) * 100) : 0}%` }} /></div>
                  </div>
                </div>
              </div>
              ...
              <div key={sk.topic} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${sk.avg >= 75 ? 'bg-emerald-500' : sk.avg >= 50 ? 'bg-amber-500' : 'bg-rose-400'}`} style={{ width: `${sk.avg}%` }} />
                  </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link to="/history" className="text-xs text-primary-600 font-bold hover:underline">View detailed →</Link>
        </div>
        {results.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>{['Quiz', 'Score', '%', 'Status', 'Rank', 'Date'].map(h => (
                  <th key={h} className="text-left text-xs font-black text-gray-400 uppercase tracking-wider px-4 py-3 whitespace-nowrap">{h}</th>
                ))}</tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {results.map(r => (
                  <tr key={r._id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3 font-black whitespace-nowrap">{r.score}/{r.totalMarks}</td>
                    <td className="px-4 py-3">
                      <span className={`font-bold ${r.percentage >= 75 ? 'text-emerald-600' : r.percentage >= 50 ? 'text-amber-600' : 'text-rose-600'}`}>{r.percentage?.toFixed(1)}%</span>
                    </td>
                    <td className="px-4 py-3"><PassBadge status={r.passStatus} /></td>
                    <td className="px-4 py-3">{r.rank ? `#${r.rank}` : '—'}</td>
                    <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{new Date(r.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                  </tr>
                ))}
              </tbody>
            </table>
