...
import { IconChart, IconBrain, IconTarget, IconCheck, IconTrophy, IconTrending, IconSearch } from '../components/Icons'
    ...
    <Layout title="Result History">
      <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
        <div>
          <h2 className="flex items-center gap-2 text-2xl font-black text-gray-900">
            <IconChart className="w-6 h-6 text-primary-600" /> Result History
          </h2>
          <p className="text-gray-500 text-sm">{results.length} total attempt{results.length !== 1 ? 's' : ''}</p>
        </div>
        <Link to="/quizzes" className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors shadow-sm">
          <IconBrain className="w-4 h-4" /> New Quiz
        </Link>
      </div>
      ...
      {results.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          <StatCard icon={<IconTarget className="w-5 h-5" />}   label="Total Attempts" value={results.length} color="blue" />
          <StatCard icon={<IconCheck className="w-5 h-5" />}    label="Passed"         value={passed}         color="green" />
          <StatCard icon={<IconTrophy className="w-5 h-5" />}   label="Best Score"     value={`${best}%`}     color="orange" />
          <StatCard icon={<IconTrending className="w-5 h-5" />} label="Average"        value={`${avg}%`}      color="purple" />
        </div>
      )}

      {results.length === 0 ? (
        <EmptyState icon={<IconTarget className="w-7 h-7" />} title="No quiz attempts yet"
          ...
          <div className="flex gap-3 mb-4 flex-wrap items-center">
            <div className="relative max-w-xs">
              ...
              <input type="text" placeholder="Search quizzes..."
                ...
                className="border border-gray-200 rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 bg-white w-full" />
            </div>
            <div className="flex rounded-xl border border-gray-200 overflow-hidden bg-white">
              {[['all', 'All'], ['pass', 'Passed'], ['fail', 'Failed']].map(([v, l]) => (
                <button key={v} onClick={() => setFilter(v)}
                  className={`px-4 py-2.5 text-sm font-bold transition-colors ${filter === v ? 'bg-primary-600 text-white' : 'text-gray-600 hover:bg-gray-50'}`}>{l}</button>
              ))}
            </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>{['#', 'Quiz', 'Score', 'Percentage', 'Status', 'Rank', 'Time', 'Date', ''].map(h => (
                    <th key={h} className="text-left text-xs font-black text-gray-500 uppercase tracking-wider px-4 py-3 whitespace-nowrap">{h}</th>
                  ))}</tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((r, i) => {
                    const pct = r.percentage
                    const gc = pct >= 75 ? 'text-emerald-600 bg-emerald-50' : pct >= 50 ? 'text-amber-600 bg-amber-50' : 'text-rose-600 bg-rose-50'
                    return (
                      <tr key={r._id} className="hover:bg-gray-50/70 transition-colors">
                        ...
                        <td className="px-4 py-3.5 font-black text-gray-900 whitespace-nowrap">{r.score}/{r.totalMarks}</td>
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`font-black text-xs px-2 py-0.5 rounded-full whitespace-nowrap ${gc}`}>{pct.toFixed(1)}%</span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3.5 text-xs text-gray-400 whitespace-nowrap">{new Date(r.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                        <td className="px-4 py-3.5">
                          <Link to={`/result/${r._id}`} className="text-xs text-primary-600 font-bold hover:underline whitespace-nowrap">Review →</Link>
                        </td>
                      </tr>
                    )
                  })}
