      ...
      {results.length === 0 ? (
        ...
        <>
            ...
            <div className="flex rounded-xl border border-white/10 overflow-hidden bg-white/5">
              {[['all','All'],['pass','Passed'],['fail','Failed']].map(([v,l])=>(
                <button key={v} onClick={()=>setFilter(v)}
                  className={`px-4 py-2.5 text-sm font-bold transition-colors ${filter===v?'bg-primary-600 text-white':'text-slate-400 hover:bg-white/5'}`}>{l}</button>
              ))}
            </div>
          ...
          <div className="bg-surface-card rounded-2xl border border-white/10 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.map((r, i) => {
                  ...
                  const gc   = pct>=75?'text-accent-400 bg-accent-500/10':pct>=50?'text-amber-600 bg-amber-500/10':'text-red-400 bg-red-500/10'
                  return (
                    <tr key={r._id} className="hover:bg-white/5 transition-colors">
                      <td className="px-4 py-3.5 text-xs font-bold text-slate-500">#{results.length-i}</td>
                      <td className="px-4 py-3.5">
                        <p className="font-bold text-white text-sm">{r.quizId?.title||'Quiz'}</p>
                        {r.quizId?.category && <p className="text-xs text-slate-500">{r.quizId.category}</p>}
                      </td>
                      <td className="px-4 py-3.5 font-black text-white">{r.score}/{r.totalMarks}</td>
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-white/10 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${pct>=75?'bg-accent-500':pct>=50?'bg-amber-500':'bg-red-500'}`} style={{width:`${pct}%`}}/>
                          </div>
                      ...
                      <td className="px-4 py-3.5 font-bold text-slate-300">{r.rank?`#${r.rank}`:'—'}</td>
                      <td className="px-4 py-3.5 font-mono text-xs text-slate-400">{fmtTime(r.timeTaken)}</td>
                      <td className="px-4 py-3.5 text-xs text-slate-500">{new Date(r.createdAt).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})}</td>
                      <td className="px-4 py-3.5">
                        <Link to={`/result/${r._id}`} className="text-xs text-primary-400 font-bold hover:underline">Review →</Link>
                      </td>
                    </tr>
                  )
                })}
            ...
            {filtered.length === 0 && <div className="text-center py-10 text-slate-500 text-sm">No results match your filter.</div>}
          </div>
        </>
      )}
    </Layout>
