...
import { IconUsers, IconCheck, IconTrending, IconTrophy, IconSearch } from '../../components/Icons'
...
export default function Leaderboard() {
  ...
  return (
    <Layout title="Leaderboard">
      {/* Hero banner */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-purple-950 rounded-3xl p-6 mb-6 text-white">
          ...
          <div className="flex items-end justify-center gap-3 mt-4">
            ...
            {top3[1] && (
              <div className="text-center flex-1 max-w-[140px]">
                <div className="w-12 h-12 bg-slate-500/30 border-2 border-slate-400/50 rounded-full flex items-center justify-center mx-auto mb-2"><RankBadge rank={2} /></div>
            ...
            {top3[0] && (
              <div className="text-center flex-1 max-w-[160px]">
                <div className="w-14 h-14 bg-yellow-500/30 border-2 border-yellow-400/60 rounded-full flex items-center justify-center mx-auto mb-2 shadow-lg shadow-yellow-500/20"><RankBadge rank={1} /></div>
            ...
            {top3[2] && (
              <div className="text-center flex-1 max-w-[140px]">
                <div className="w-12 h-12 bg-amber-700/30 border-2 border-amber-600/50 rounded-full flex items-center justify-center mx-auto mb-2"><RankBadge rank={3} /></div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {[[IconUsers,leaderboard.length,'Total Attempts'],[IconCheck,passed,'Passed'],[IconTrending,`${avgPct}%`,'Avg Score'],[IconTrophy,top3[0]?.score||'—','Top Score']].map(([Ic,v,l])=>(
        ...
        <div className="p-4 border-b border-gray-100 flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-0 max-w-xs">
            ...
            <input type="text" placeholder="Search students..." value={search} onChange={e=>setSearch(e.target.value)}
              className="w-full border border-gray-200 rounded-xl pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"/>
          </div>
          <div className="flex rounded-xl border border-gray-200 overflow-hidden bg-gray-50">
            {[['all','All'],['pass','Pass'],['fail','Fail']].map(([v,l])=>(
              <button key={v} onClick={()=>setFilter(v)}
                className={`px-4 py-2 text-xs font-bold transition-colors ${filter===v?'bg-primary-600 text-white':'text-gray-600 hover:bg-gray-100'}`}>{l}</button>
            ))}
          </div>
          <select value={sort} onChange={e=>setSort(e.target.value)}
            className="border border-gray-200 rounded-xl px-3 py-2 text-xs font-semibold text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-primary-500">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((e, i) => {
                  ...
                  return (
                    <tr key={i} className={`transition-colors ${isMe?'bg-primary-50/70 border-l-4 border-primary-500':'hover:bg-gray-50/80'}`}>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary-500 to-indigo-600 flex items-center justify-center text-white font-black text-sm flex-shrink-0">
                            {(e.name||'?')[0].toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-gray-900 text-sm">{e.name}{isMe&&<span className="ml-1.5 text-xs bg-primary-100 text-primary-700 px-1.5 py-0.5 rounded-full font-black">You</span>}</p>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${e.percentage>=75?'bg-emerald-500':e.percentage>=50?'bg-amber-500':'bg-rose-500'}`}
      ...
      <div className="mt-4">
        <Link to="/quizzes" className="text-sm text-primary-600 hover:underline font-semibold">← Back to Quizzes</Link>
      </div>
    </Layout>
  )
}
