...
import {
  IconClock, IconCheck, IconBrain, IconHelp, IconTrophy, IconTarget,
  IconInfo, IconAlertTriangle, IconRocket,
...
const TimerDisplay = memo(function TimerDisplay({ seconds }) {
  ...
  const s   = seconds % 60
  const cls = seconds < 300 ? 'text-rose-500' : seconds < 600 ? 'text-amber-500' : 'text-emerald-500'
  return (
  ...
  return (
    ...
    >
      <span className={[
        ...
        selected ? 'bg-primary-600 text-white shadow' : 'bg-gray-100 text-gray-600',
      ].join(' ')}>
        {label}
      </span>
      <span className={`text-sm font-medium leading-relaxed flex-1 text-left break-words ${selected ? 'text-primary-800' : 'text-gray-700'}`}>
        {text}
      </span>
      {selected && <IconCheck className="ml-auto w-5 h-5 text-primary-500 flex-shrink-0" />}
    </button>
  )
})
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 max-w-sm w-full text-center">
          ...
          <p className="text-sm text-rose-600 font-medium mb-6 bg-rose-50 border border-rose-200 rounded-xl px-4 py-3">
            {errorMsg}
          </p>
          <button
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-indigo-950 p-6">
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8">
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            {[
              [IconHelp, total,                         'Questions'],
              [IconTrophy, quiz?.totalMarks,              'Total Marks'],
              [IconClock, `${quiz?.duration}m`,          'Time Limit'],
              [IconTarget, `${quiz?.passMarks} (${quiz?.passPercentage}%)`, 'Pass Marks'],
            ].map(([Ic, v, l]) => (
              <div key={l} className="bg-primary-50 border border-primary-100 rounded-xl p-3 text-center">
          ...
          {quiz?.attemptsLeft !== undefined && (
            <p className="text-center text-xs text-gray-500 mb-4 font-medium">
              Attempts remaining: <strong className="text-primary-600">{quiz.attemptsLeft}</strong> / {quiz.attemptsAllowed}
            </p>
          )}

          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-left">
            <p className="flex items-center gap-1.5 text-xs font-black text-amber-800 mb-2 uppercase tracking-wide"><IconInfo className="w-3.5 h-3.5" /> Instructions</p>
            <ul className="text-xs text-amber-700 space-y-1.5 leading-snug">
              ...
              {quiz?.negativeMarking && (
                <li className="flex items-center gap-1.5 font-bold text-rose-700">
                  <IconAlertTriangle className="w-3 h-3 flex-shrink-0" /> Negative marking: −{quiz.negativeMarksPerQ} mark per wrong answer
                </li>
              )}
            </ul>
          </div>
          ...
          <button
            onClick={startQuiz}
            className="w-full flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-black py-4 rounded-2xl text-lg transition-all shadow-lg hover:shadow-xl"
          >
            <IconRocket className="w-5 h-5" /> Begin Quiz
          </button>
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <div className="w-16 h-16 rounded-2xl bg-primary-50 text-primary-600 flex items-center justify-center mx-auto mb-4 animate-bounce">
        ...
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 flex-shrink-0">
            ...
            <button
              ...
              className="bg-primary-600 hover:bg-primary-700 text-white text-sm font-black px-5 py-2.5 rounded-xl transition-colors shadow-sm"
            >
              Submit
            </button>
          </div>
      ...
      {errorMsg && (
        <div className="max-w-5xl mx-auto w-full px-4 pt-4">
          <div className="flex items-center gap-2 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl px-4 py-3 text-sm font-semibold">
            <IconAlertTriangle className="w-4 h-4 flex-shrink-0" /> {errorMsg}
          </div>
        </div>
      )}
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-gray-500 font-medium">
              <span className="text-primary-600 font-black">{answered}</span> / {total} answered
            </span>
            <button
              type="button"
              onClick={next}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sticky top-24">
            ...
            <button
              type="button"
              onClick={() => setConfirm(true)}
              className="w-full bg-primary-600 hover:bg-primary-700 text-white text-sm font-black py-3 rounded-xl transition-colors"
            >
              Submit Quiz
            </button>
          </div>
