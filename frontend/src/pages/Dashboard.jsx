...
export default function Dashboard() {
  ...
  return (
    <Layout title="Dashboard">
      {/* Welcome hero */}
      <div className="bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 rounded-3xl p-6 mb-6 text-white relative overflow-hidden">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-3 flex-wrap">
            <Link to="/quizzes" className="bg-white text-blue-700 font-black px-5 py-3 rounded-xl hover:bg-blue-50 transition-all shadow-lg text-sm">
              🧠 Take Quiz
            </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h3 className="font-black text-gray-900 text-sm">🧠 Available Quizzes</h3>
            <Link to="/quizzes" className="text-xs text-blue-600 hover:underline font-bold">View all →</Link>
          </div>
          {quizzes.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-gray-400 text-sm">No quizzes available yet.</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-50">
              {quizzes.map(q => (
                <div key={q._id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50/60 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{q.totalQuestions}Q · {q.totalMarks}M · {q.duration}min</p>
                  </div>
                  <div className="flex-shrink-0">
                    {q.attempted ? (
                      <div className="text-right">
                        <PassBadge status={q.myStatus} />
                        <p className="text-xs text-gray-400 mt-0.5">{q.myPct?.toFixed(0)}%</p>
                      </div>
                    ) : (
                      <Link to={`/quiz/${q._id}`} className="text-xs bg-blue-600 hover:bg-blue-700 text-white font-black px-3 py-2 rounded-xl transition-colors">Start →</Link>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <Link to="/history" className="text-xs text-blue-600 hover:underline font-bold">View all →</Link>
          </div>
          {results.length === 0 ? (
            <div className="text-center py-10">
              <div className="text-4xl mb-3">🎯</div>
              <p className="text-gray-500 text-sm">No quiz attempts yet.</p>
              <Link to="/quizzes" className="mt-3 inline-block bg-blue-600 text-white text-xs font-bold px-4 py-2.5 rounded-xl">Take your first quiz</Link>
            </div>
          ) : (
            <div className="divide-y divide-gray-50">
                ...
                return (
                  <Link to={`/result/${r._id}`} key={r._id}
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-gray-900 text-sm truncate">{r.quizId?.title||'Quiz'}</p>
                      <p className="text-xs text-gray-400">{new Date(r.createdAt).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'})} {r.rank&&`· Rank #${r.rank}`}</p>
                    </div>
                    <div className="flex items-center gap-2 ml-3 flex-shrink-0">
                      <div className="text-right">
                        <p className="font-black text-sm text-gray-900">{r.score}/{r.totalMarks}</p>
                        <PassBadge status={r.passStatus} />
                      </div>
                      <span className={`text-sm font-black px-2.5 py-1.5 rounded-xl ${pct>=75?'bg-emerald-100 text-emerald-700':pct>=50?'bg-amber-100 text-amber-700':'bg-red-100 text-red-600'}`}>
                        {pct.toFixed(0)}%
                      </span>
                    </div>
                  </Link>
      ...
      {/* Bottom CTA row */}
      {total > 0 && (
        <div className="mt-6 bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4 text-white">
          <div>
          ...
          <div className="flex gap-3">
            <Link to="/quizzes"   className="text-sm bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 py-2.5 rounded-xl transition-colors">🧠 New Quiz</Link>
